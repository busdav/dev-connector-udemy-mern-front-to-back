...
router.get('/user/:user_id', async (req, res) => {
  try {
    ...
    }).populate('user', ['name', 'avatar']); // Why not `req.user` here? Because `req.user` comes from auth, while this here is a public route
...
// @route   DELETE api/profile/experience/:exp_id (This could also be a PUT request since we're updating, but prefer to make it DELETE req)
...
router.delete('/experience/:exp_id', auth, async (req, res) => {
  try {
    ...
    const removeIndex = profile.experience
      .map(item => item.id)
      .indexOf(req.params.exp_id); // `map` returns new array consisting of the IDs, `indexOf` returns the index of the desired ID

    profile.experience.splice(removeIndex, 1); // At removeIndex, remove 1 item
...
router.delete('/education/:edu_id', auth, async (req, res) => {
  try {
    ...
    const removeIndex = profile.education
      .map(item => item.id)
      .indexOf(req.params.edu_id); // `map` returns new array consisting of the IDs, `indexOf` returns the index of the desired ID

    profile.education.splice(removeIndex, 1); // At removeIndex, remove 1 item
